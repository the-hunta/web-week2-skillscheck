...
//  let midddleNums = numbers.slice(1,2)
...
// let total = myNumbers.reduce(function(num){
// 	return num
// })
...
// Last, lets use .forEach to find the index of each item in the array. First, create an empty array called myNumbersIndex. Next, using forEach, push each items index from the myNumbers array to the myNumbersIndex array
 ...
 let myNumbersIndex = [];
 myNumbers.forEach(function(elem){
	 myNumbersIndex.push(elem);
 });
...
let enemies = people.filter(function(freind){
	return people.freind = false;
})
